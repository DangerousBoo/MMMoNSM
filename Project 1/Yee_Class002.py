...
class SimulationRunner:
    ...
    def execute(frame_skip=3, **kwargs):
        ...
        recorder_count = getattr(config, "recorder_count", 1)
    ...
    def plot_2d_animation(results, fps=40):
        ...
        if cfg.recorder_count >= 2:
            scat3 = ax.scatter(nodes_x[cfg.x_diag], nodes_y[cfg.y_diag], color='cyan', s=30, zorder=2, marker='x', label='Obs Diag')
            scat4 = ax.scatter(nodes_x[cfg.x_before], nodes_y[cfg.y_before], color='yellow', s=30, zorder=2, marker='x', label='Obs Before')
        scat5 = ax.scatter(nodes_x[cfg.x_after], nodes_y[cfg.y_after], color='magenta', s=30, zorder=2, marker='x', label='Obs After')
        ...
        def update(i):
            ...
            if cfg.recorder_count >= 2:
                ...
                return quad, time_text, scat1, scat2, scat5
    ...
    def plot_error_analysis(results):
        ...
        ax.axvline(cfg.f_min, color='green', ls=':', alpha=0.5, label=f'f_min ({cfg.f_min:.2e} Hz)') 
        ax.set_xscale('log')
        ax.set_ylim(min_phase_error * 1.2, max_phase_error * 1.2)
...
if __name__ == "__main__":
    ...
    results = SimulationRunner.run_full_analysis(
        ...
        do_hankel = True,
        recorder_count = 1 #3 for the 3 recorders, 1 for 1 wowie
    )
