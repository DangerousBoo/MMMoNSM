...
results_barrier = SimulationRunner.execute(V_DC=0.0, T_total=250.0e-15, E_target=0.55*1.602e-19, frame_skip=200)
results_free = SimulationRunner.execute(V0=0.0, V_DC=0.0, T_total=250.0e-15, E_target=0.55*1.602e-19, frame_skip=200, dt=results_barrier["config"].dt)
...
T = np.abs(fft_J_bar) / (np.abs(fft_J_free))
